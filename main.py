"""
Created on Tue Dec  7 08:17:07 2021
...
from queryTemps import queryLabel,query_template1,query_template2,query_template3
...
class mainPro:
    ...
    ## parse the message and get relation
    def parseRel(self,entity,graph,WDT):
        gr = getRelation()
        relationList,tbd = gr.getRel(self.message)
        print("parseRel",entity, relationList,tbd)
        if tbd==True:
            relationList = gr.tbdRel(relationList)
        if 'recommend' not in relationList:
            relList = []
            for idx, relation in enumerate(relationList):
                
                relURI = gr.getRelURI(graph, relation,WDT)
                ...
                # if not, then query alias
                    ...
                    relAliasList = gr.searchAlias(relation,predAlias)
    ...
    def parseMsg(self,graph,WDT,WD,images):
        ...
        if len(entities)==1 and len(relations)==1 and 'recommend' not in relations and notPicture:
            ...
            incrowd, rate, lbl,cnt1,lblRev,cnt2 = checkCrowdER(entity,relation,graph, WDT, WD, crowd.cleanCrowd, crowd.aggAns, crowd.numCnt, crowd.irate)
            print('crowd')
            print('crowd, rate, lbl,cnt1,lblRev,cnt2: ',incrowd, rate, lbl,cnt1,lblRev,cnt2)
            print(entity,relation)
            gr = getRelation()
            relid = gr.getRelWDTid(graph, WDT, relation)
            res = []
            ...
            if len(res)==0:
                ...
                if incrowd:
                    answer_template = "Hi, {} of {} is {}, according to the crowd, who had an inter-rater agreement of {} in this batch; the answer distribution for this task was {} support votes and {} reject vote. ".format(relation,entity,answers,rate,cnt1,cnt2)
                else:
        ...
        if len(entities)==2 and notPicture:
            ...
            print(res, len(res))
            incrowd, rate, lbl,cnt1,lblRev,cnt2 = checkCrowdER(entities[1],entities[0],graph, WDT, WD, crowd.cleanCrowd, crowd.aggAns, crowd.numCnt, crowd.irate)
        ...
        if 'recommend' in relations and len(entities)==1:
            ...
            if human:
                ge = getEntity()
                pos_tokens = ge.getTokens(self.message)
                target = ge.returnNAfRcmd(pos_tokens)
                if label == 'POSITIVE':
        ...
        if not notPicture:
            ...
            if not noPoster:
                ...
                if len(entities) != 0:
                    ...
                    ge = getEntity()
                    tokens = ge.getTokens(self.message)
                    genre = ge.returnNounBfMovie(tokens)
